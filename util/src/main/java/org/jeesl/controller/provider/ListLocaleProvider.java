...
public class ListLocaleProvider <LOC extends JeeslLocale<?,?,LOC,?>>
									implements JeeslLocaleManager<LOC>,JeeslLocaleProvider<LOC>
{
	private static final long serialVersionUID = 1L;

	final static Logger logger = LoggerFactory.getLogger(ListLocaleProvider.class);
	...
	public static <LOC extends JeeslLocale<?,?,LOC,?>> ListLocaleProvider<LOC> instance(List<LOC> locales) {return new ListLocaleProvider<>(locales);}
	
	public ListLocaleProvider()
	{
		localeCodes = new ArrayList<>();
		mapLocales = new HashMap<String,LOC>();
		locales = new ArrayList<>();
	}
	public ListLocaleProvider(List<LOC> locales)
	{
		this();
