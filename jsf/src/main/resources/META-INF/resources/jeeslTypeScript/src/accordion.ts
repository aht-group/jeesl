(function($) {
    ...
    function expandSection(element: JQuery): void {
        element.attr('data-collapsed', 'false');
        element.stop().animate({ height: (element.prop('scrollHeight') as number + 2) + 'px' }, 1000);
    }
    ...
    const root = $('.html-accordion .html-container > div');
    const mainSection = root.children().first();
