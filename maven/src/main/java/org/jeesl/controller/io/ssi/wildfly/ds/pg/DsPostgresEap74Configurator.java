...
public class DsPostgresEap74Configurator extends AbstractEapDsConfigurator implements EapDsConfigurator
{
	final static Logger logger = LoggerFactory.getLogger(DsPostgresEap74Configurator.class);

	private DsPostgresEap71Configurator ds71Configurator;
	
	public static DsPostgresEap74Configurator instance(ModelControllerClient client) {return new DsPostgresEap74Configurator(client);}
	private DsPostgresEap74Configurator(ModelControllerClient client)
	{
		super(client);
	...
	{			
		...
		if(!moduleXml.exists())
		{
			FileUtils.copyInputStreamToFile(mrl.searchIs(AbstractEapDsConfigurator.srcBaseDir+"/eap/7.4/postgres.xml"), moduleXml);
			...
			FileUtils.copyFileToDirectory(MavenArtifactResolver.resolve("net.postgis:postgis-geometry:2.5.0"),moduleXml.getParentFile());
	...
	public void moduleHibernate() throws IOException
	{
		...
		byte[] jeeslXml = IOUtils.toByteArray(mrl.searchIs(AbstractEapDsConfigurator.srcBaseDir+"/eap/7.4/hibernate.xml"));
		byte[] jbossXml = Files.readAllBytes(moduleXml.toPath());
		
		if(!Arrays.equals(jeeslXml,jbossXml))
		{
			Files.write(moduleXml.toPath(),jeeslXml);
			FileUtils.copyFileToDirectory(MavenArtifactResolver.resolve("org.hibernate:hibernate-spatial:5.3.28.Final"),moduleXml.getParentFile());
