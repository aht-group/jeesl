...
public class DsPostgresEap73Configurator extends AbstractEapDsConfigurator implements EapDsConfigurator
{
	...
	public static DsPostgresEap73Configurator instance(ModelControllerClient client) {return new DsPostgresEap73Configurator(client);}
	private DsPostgresEap73Configurator(ModelControllerClient client)
	{
		super(client);
	...
	{			
		...
		if(!moduleXml.exists())
		{
			FileUtils.copyInputStreamToFile(mrl.searchIs(AbstractEapDsConfigurator.srcBaseDir+"/eap/7.3/postgres.xml"), moduleXml);
	...
	public void moduleHibernate() throws IOException
	{
		...
		if(!Arrays.equals(jeeslXml,jbossXml))
		{
			...
			FileUtils.copyFileToDirectory(MavenArtifactResolver.resolve("org.hibernate:hibernate-spatial:5.3.18.Final"),moduleXml.getParentFile());
			...
			FileUtils.copyFileToDirectory(MavenArtifactResolver.resolve("org.hibernate.common:hibernate-commons-annotations:5.1.0.Final"),moduleXml.getParentFile());
